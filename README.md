...
**First-time setup (non-Docker / custom env):** Set `CIVICSORT_BOOTSTRAP_ADMIN=1`, start the server, register the first Operations Admin user, then unset the variable. In the provided `docker-compose.yml`, bootstrap is enabled for the empty DB case.
